...
def save_plain_text_content(email_bytes, email_id):
    ...
    if msg.is_multipart():
        ...
        if msg.get_content_type() == "text/plain":
            ...
            text_content = get_text_from_html(
                msg.get_payload(decode=True).decode(msg.get_content_charset("utf-8"))
            )
...
def search_and_process_emails(
    imap_client, email_source, search_keyword, start_date, end_date
):
    search_criteria = "ALL"
    ...
    if search_keyword:
        search_criteria += (
            f' BODY "{search_keyword}"'  # Ensure the correct combination of conditions
        )
    ...
    if typ == "OK":
        ...
        for num in email_ids:
            ...
            if typ == "OK":
                ...
                print(
                    f"Downloading and processing email ID: {email_id} from {email_source}"
                )
                save_plain_text_content(email_data[0][1], email_id)
            else:
                print(
                    f"Failed to fetch email ID: {num.decode('utf-8')} from {email_source}"
                )
    else:
        print(
...
def main():
    ...
    parser.add_argument(
        "--startdate", help="Start date in DD.MM.YYYY format.", required=False
    )
    parser.add_argument(
        "--enddate", help="End date in DD.MM.YYYY format.", required=False
    )
    ...
    if args.startdate and args.enddate:
        try:
            start_date = datetime.strptime(args.startdate, "%d.%m.%Y").strftime(
                "%d-%b-%Y"
            )
            end_date = datetime.strptime(args.enddate, "%d.%m.%Y").strftime("%d-%b-%Y")
        except ValueError as e:
            print(
                f"Error: Date format is incorrect. Please use DD.MM.YYYY format. Details: {e}"
            )
            return
    ...
    # Search and process emails from Gmail
